...
V1a =[-17 10 6 17 -13]; %初始速度
V1b =[-17 10 6 17 -13]; %结束速度
v2 =[-17 10 6 17 -13];  %转弯速度
V3a =[-17 10 6 17 -13];  %第三阶段初始速度
V3b =[-17 10 6 17 -13];  %第三阶段结束速度
R3 = [100 0 0 0 59];
R2 = R1b+v2./abs(v2).*abs(m2-m1);  %转弯结束纵坐标
t=0.0010;  %单位s 精度0.1ms
RCS=[5 10 30 30 30 5+rand*(20.0-5.0) 5+rand*(20.0-5.0) 5+rand*(20.0-5.0) 5+rand*(20.0-5.0) 5+rand*(20.0-5.0)];
% RCS=30;
...
strx=['径向距离/m','运动方向/°','径向速度/m/s','角度/°','RCS/dBsm'];
str1=[56.550,0.000,0.000,0.000,16.0];

fprintf(fidout6,'%16.8f',strx);
fprintf(fidout6,'\r\n');
for n1=1:1:5000
fprintf(fidout6,'%16.3f', str1);
fprintf(fidout6,'\r\n');
end
for n1=1:1:5000
...
V1nt = V1n.*cos(det);
     ...
     fprintf(fidout1,'%22.3f', R1tn(1),angBAY(1),V1nt(1),det(1),RCS(1));
     fprintf(fidout1,'\r\n');
%      fprintf(fidout6,'%22.3f', str1);
end
end
if count>=2
for n1=1:1:N1(2)
