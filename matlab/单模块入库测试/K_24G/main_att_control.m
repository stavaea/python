...
%% parameter set
number               = 'rs_k_up_230914002'; %  rs_k_down_230914003
mode         = 1     ;                        % 模式选择 mode == 1  发射 mode == 0 接收
% % tpye         = 'RS_up';                        % 测试模块名称
% tpye         = 'RS_down';                      % 测试模块名称
com_num      = 'com4';                          % 串口号
bps          = 1000000;                         % 波特率
att_add_down = hex2dec('21');                   % 下变频衰减器地址
att_down     = (0:0.25:30)/0.125;               % 下变频衰减器衰减范围
...
if ~mode
    ...
    amp_set              = -45;
    fre_set              = linspace(fre_start,fre_end,11) ;
    spec_fre_set         = fre_set + Lf_freq * 2 -(23000 - 22999.997026);
...
if mode == 1
    ...
    clock1 = clock;
    savefile = strcat('data\att_up\',sprintf('%04d%02d%02d_%02d%02d%02.0f_RF_att_down_%s',clock1(1),clock1(2), ...
        clock1(3),clock1(4),clock1(5),clock1(6),number))
    save(savefile);
end
%% close
