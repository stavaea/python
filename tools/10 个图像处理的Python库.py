...
'''OpenCV无疑是最流行的图像处理库之一。它最初由英特尔公司开发，已被广泛应用于计算机视觉领域。
它支持无数与计算机视觉和机器学习相关的算法，这有助于理解视觉数据并做出有见地的决策。
OpenCV还针对实时应用进行了高度优化，使其成为视频监控，自动驾驶汽车和先进机器人的绝佳选择。
...
它还包括各种形态操作，如侵蚀，扩张和连接成分分析。这些操作是图像二值化、去噪和形状分析等任务的基础。
这些功能OpenCV 都有，但是Mahotas更专注于图像的图像处理，而并不像OpenCV那样什么都有，所以，Mahotas的API更简单、也更友好。
并且学习起来也比OpenCV简单，但是速度方面却差不多。'''
# 这是一个简单的示例（使用 mahotas 附带的示例文件），使用上述阈值区域作为种子调用分水岭（我们使用 Otsu 定义阈值）。
...
# 4、Scikit-Image
'''Scikit-Image建立在Scikit-Learn机器学习库的基础上的扩展功能，包括更高级的图像处理能力。
所以如果已经在使用Scikit进行ML，那么可以考虑使用这个库。
它提供了一套完整的图像处理算法。它支持图像分割、几何变换、色彩空间操作和过滤。
与许多其他库不同，Scikit-Image支持多维图像，这对于涉及视频或医学成像的任务是很有帮助的。
Scikit-Image与其他Python科学库(如NumPy和SciPy)无缝集成。'''
...
SimpleCV的设计目标是使计算机视觉技术对于初学者和非专业人士也能更加可靠和易于使用。
它提供了一个简单的API，隐藏了底层的复杂性，使用户能够快速实现常见的计算机视觉任务。
...
# 10、timm
'''timm是一个PyTorch模型库，虽然可能和图像处理没有关系，但是它提供了广泛的预训练模型和计算机视觉模型的集合，这对我们来进行深度学习的时候是非常有帮助的。
现在它已经是huggingface的子项目了，这意味着这个项目有了资金的支持，所以不会担心发展的问题。'''
import timm
import torch
