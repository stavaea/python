...
class ChatRoom(object):
    def connect(self):    #配置连接
        ...
        print ('从%s成功连接到%s' % (self.s.getpeername(), self.s.getpeername()))
        return self.s

    def revice(self, s):    #监听消息
        self.my = [s]
        while True:
            ...
            if s in rs:
                try:
                    ...
                    if type == 'login':    #登录逻辑
                        ...
                        if 'yes' == isMy:    #如果是自己发的消息，字体用DarkTurquoise，如果是别人发的消息，字体用Black
    ...
    def chat_interface(self, nickname):    #进入聊天界面
        client_draw.draw_chat(self, nickname)

    def return_login_interface(self):    #返回登录界面
        self.label_nickname.destory()    #将不需要的label_nickname控件先销毁
        self.input_nickname.destory()    #将不需要的input_nickname控件先销毁
        self.label_password.destory()    #将不需要的label_password控件先销毁
        self.input_password.destory()    #将不需要的input_password控件先销毁
    ...
    def on_closing(self):    #聊天界面，点击右上角退出时执行
    ...
    def main():
        ...
        t = threading.Thread(target=chatRoom.revice, args=(client,))    #创建一个线程，监听消息
        t.start()
        chatRoom.root = tkinter.Tk()    #创建主窗口，用于容纳其他组件
