...
class MY_GUI():
    def __init__(self, init_window_name):
        ...
        time.sleep(1)    #休眠1秒
        # 测试网卡是否属于断开状态
        assert self.ifaces.status() in [const.IFACE_DISCONNECTED, const.IFACE_INACTIVE]

    def __str__(self):
        # 自动会调用的函数,返回自身的网卡
        return '(WIFI: %s, %s)' % (self.wifi, self.ifaces.name())
    ...
    def set_init_window(self):
        ...
        self.search = Button(labelframe, text='搜索附加WiFi', command=self.scans_wifi_list).grid(column=0, row=0)
        ...
        self.file = Button(labelframe, text='添加密码文件路径', command=self.add_mm_file).grid(column=2, row=1)
        ...
        self.wifi_treeconfigure(yscrollcommand=self.vbar.set)
        # 表格的标题
        ...
        self.wifi_tree.heading('d', text='singal')
        self.wifi_tree.grid(row=4, column=0, sticky=NSEW)
        self.wifi_tree.grid('<Double-1>', self.onDBClick)
    ...
    def show_scans_wifi_list(self, scans_res):
        for index, wifi_info in enumerate(scans_res):
            self.wifi_tree.insert('', 'end', values=(index + 1, wifi_info.ssid, wifi_info.bssid, wifi_info.singal))
    ...
    def connect(self, pwd_Str, wifi_ssid):
        ...
        self.tmp_profile = self.iface.add_network_profile(self.profile)    #创建新的连接文件
        self.iface.connect(self.tmp_profile) #连接
        time.sleep(5)
